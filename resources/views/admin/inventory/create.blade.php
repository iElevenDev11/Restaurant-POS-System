        ...
        <form action="{{ route('inventory.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="number" name="quantity" id="quantity" value="{{ old('quantity', 0) }}" step="0.01" min="0"
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50" required>
                </div>

                <div>
                    <label for="unit" class="block text-sm font-medium text-gray-700 mb-1">Unit</label>
                    <input type="text" name="unit" id="unit" value="{{ old('unit') }}" placeholder="e.g., kg, liters, pieces"
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50" required>
                </div>
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="number" name="reorder_level" id="reorder_level" value="{{ old('reorder_level', 10) }}" step="0.01" min="0"
                       class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50" required>
                <p class="text-sm text-gray-500 mt-1">The minimum quantity at which you should reorder this item</p>
            </div>

            <div class="flex justify-end">
                <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Add Inventory Item</button>
            </div>
        </form>
    </div>
</div>
